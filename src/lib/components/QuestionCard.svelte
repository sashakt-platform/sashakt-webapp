		...
		<Card.Content class="p-5 pt-1">
			...
			{:else if question.question_type === question_type_enum.NUMERICALINTEGER || question.question_type === question_type_enum.NUMERICALDECIMAL}
				{#if isLocked}
					...
					{@const correctAnswerVal = currentSelection?.correct_answer}
					<div
					...
					{#if isCorrect === false}
						...
						>
							<p class="w-full text-sm whitespace-pre-wrap">{correctAnswerVal}</p>
							{@render showCorrectWrongMark('correct')}
						</div>
					{/if}
				{:else}
			...
			{:else if question.question_type === question_type_enum.MATRIXRATING}
				{@const matrixOpts = options as unknown as TMatrixOptions}
				<div class="overflow-x-auto">
						...
						<thead>
							<tr>
								...
								{#each matrixOpts.columns.items as col (col.id)}
									<th
										class="border border-gray-300 bg-gray-100 px-4 py-3 text-center font-semibold"
									>
										{col.key} – {col.value}
									</th>
								{/each}
...
{:else}
	<OmrCard
		...
		{selectedOptionIds}
		{isSelected}
		onSelectOption={(optionId, isRemoving) =>
			handleSelection(question.id, optionId, isRemoving ?? false)}
		{candidateInput}
		{hasUnsavedChanges}
		{hasSavedBefore}
		onSubjectiveSubmit={handleSubjectiveSubmit}
		onCandidateInputChange={(v) => (candidateInput = v)}
		{matrixSelections}
		{matrixResponse}
